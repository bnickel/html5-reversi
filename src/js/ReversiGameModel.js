...
{
	this.parent.constructor.call(this, rows, columns);
	this.__blackScore            = 0;
	this.__whiteScore            = 0;
	this.__emptyScore            = 0;
...
ReversiGameModel.inheritsFrom(GameModel);

ReversiGameModel.prototype.newGame = function(firstTurn)
{
	this.parent.newGame.call(this, firstTurn);
...
{
	this.__safeDiscs = undefined;
	this.parent.setBoard.call(this, board);
}

ReversiGameModel.prototype.onBoardChanged = function(eventArgs)
{
	this.updateStats();
	this.parent.onBoardChanged.call(this, eventArgs);
}

ReversiGameModel.prototype.onMove = function(eventArgs)
{
	this.parent.onMove.call(this, eventArgs);
...
{
	return this.__safeDiscs != undefined ? this.__safeDiscs.getPiece(row, column) : false;
}

ReversiGameModel.prototype.setSafeDisc = function(row, column, value)
{
	if(this.__safeDiscs == undefined)
		this.__safeDiscs = new Board(this.getRows(), this.getColumns(), false);
...
{
	this.__useComplexStats = value ? true : false;
	if(value)
		this.updateStats();
}
...
{
	this.__supressTurnValidation = value ? true : false;
}
...
ReversiGameModel.prototype.clone = function(model)
{
	if(model == undefined)
		model = new ReversiGameModel(1,1);
	
	model = this.parent.clone.call(this, model);
	model.__blackScore            = this.__blackScore;
	model.__whiteScore            = this.__whiteScore;
	model.__emptyScore            = this.__emptyScore;
	model.__blackFrontierCount    = this.__blackFrontierCount;
	model.__whiteFrontierCount    = this.__whiteFrontierCount;
	model.__blackSafeCount        = this.__blackSafeCount;
	model.__whiteSafeCount        = this.__whiteSafeCount;
	model.__useComplexStats       = this.__useComplexStats;
	model.__supressTurnValidation = this.__supressTurnValidation;
	return model;
}