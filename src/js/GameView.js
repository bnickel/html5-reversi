...
function GameView(canvasSelector, model) {
    ...
    this.__canvasElement  = document.querySelector(canvasSelector);
    this.__model          = model;
    this.__visualPieces   = [];
    ...
    this.getCanvasElement().addEventListener('click',     function(event) { self.onClick(event);     }, false);
    this.getCanvasElement().addEventListener('mousemove', function(event) { self.onMouseMove(event); }, false);
    this.getCanvasElement().addEventListener('mouseout',  function(event) { self.onMouseOut(event);  }, false);
    this.getCanvasElement().addEventListener('mousein',   function(event) { self.onMouseMove(event); }, false);
    
    this.getModel().addEventListener('boardchanged',       function(event) { self.updateDisplay();             }, false);
    this.getModel().addEventListener('simulationchanged',  function(event) { self.updateDisplay();             }, false);
    this.getModel().addEventListener('interactivechanged', function(event) { self.onInteractiveChanged(event); }, false);
    this.getModel().addEventListener('gameover',           function(event) { self.onGameOver(event);           }, false);
}
...
GameView.prototype.queueDrawing = function(piece) {
    ...
    this.__redrawInterval || (this.__redrawInterval = setInterval(function () { self.draw(); }, 10));
};
